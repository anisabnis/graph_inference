...
for i in range(6):
    ...
    for n in ['bandcon', 'att', 'sanet']:
        accuracy_by_error[i].append(ped[n][i])
...
p6 = ax.bar(ind +4*width, fifth, width,color='y')
...
plt.grid()
ax.legend((p1[0], p2[0], p3[0], p4[0], p5[0], p6[0]),  ('0 \% error', '10 \% error', '20 \% error', '30 \% error', '40 \% error', '50 \% error') , prop={'size':11},  bbox_to_anchor=(0.5, -0.1), loc=9, borderaxespad=0. ,fontsize=30, ncol=3)
ax.grid()

plt.savefig('experiment' + str('II_ped') + '.pdf')
